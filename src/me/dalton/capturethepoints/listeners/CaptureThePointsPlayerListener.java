...
//            plugin.playerData.get(p).money = 100000;
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (this.ctp.playerData.get(event.getPlayer()) != null && !ctp.playerData.get(event.getPlayer()).isInLobby) {
            if (isInside(loc.getBlockY(), 0, 128) && isInside(loc.getBlockX(), ctp.mainArena.x1, ctp.mainArena.x2) && isInside(loc.getBlockZ(), ctp.mainArena.z1, ctp.mainArena.z2) && loc.getWorld().getName().equalsIgnoreCase(ctp.mainArena.world)) {
                return;
            } else {
